...
## Phase 4: 사용자 프로파일 인터뷰 (핵심 — config는 여기서 유도된다)

> 앞의 Phase 1~3은 **기술 설정**(경로·MCP)이다. 이 Phase가 셋업의 본론 — **"당신이 무슨 일을 어떻게 하는지"를 알아야 시스템이 당신에게 맞게 굽는다.** 저장 노트의 상세수준·구조·연결을 매번 묻는 대신 여기서 한 번 캐서 기본값으로 굳힌다.

### 인터뷰 방식 (중요)
- **대화형 — AskUserQuestion 옵션 클릭 ❌.** 각 앵커를 평문으로 던지고, 답변마다 **적응형 후속 1~2개**를 되묻는다("왜?", "어떤 상황에서?", "최근 실제 사례 하나만"). 옵션형은 후속 분기가 불가능해 프로파일링이 안 된다.
- **첫 문장 고지**: "평가나 심문이 아니에요 — 당신을 알아야 정리·검색이 당신 방식에 맞게 작동합니다. 10~15분, 중간에 멈춰도 괜찮아요."
- 중간 이탈 허용 → 부분 프로필도 저장(`completeness: partial`).

### 앵커 질문 6종 (순서대로, 각 답변에 후속 되묻기)

| # | 앵커 | 캐는 것 | 후속 예시 |
|---|---|---|---|
| A | "요즘 시간을 가장 많이 쓰는 일 2~3가지는? 각각 주당 몇 시간쯤?" | 업무 정체성·비중 | "그중 가장 부담스러운 건? 왜?" |
| B | "그중 하나를 시작부터 끝까지 순서대로 — 쓰는 도구와 나오는 결과물 포함해서 얘기해 주세요." | 워크플로 해부 | "그 단계에서 손으로 옮기는 게 있나요?(복붙·정리·변환)" |
| C | "그 흐름에서 제일 지겹거나 자주 막히는 지점은? **최근 실제로 짜증 났던 일 하나**만." | 페인포인트(사례 강제) | "그때 얼마나 걸렸나요? 없었으면 뭘 했을까요?" |
| D | "매주 반복하는 일 중 '이건 기계가 해도 될 텐데' 싶은 게 있나요?" | 자동화 후보 | "그걸 안 맡기는 이유는?(신뢰? 방법을 몰라서?)" |
| E | "자료를 어디에 모으고 나중에 어떻게 다시 찾나요? **못 찾아서 다시 만든 적** 있나요?" | 정보 흐름·지식관리 습관 | "주로 어떤 형태?(글·링크·PDF·대화 캡처)" |
| F | "3개월 뒤 이 시스템이 해줬으면 하는 것 딱 하나는?" | 성공 기준 | "그게 되면 시간이 얼마나 돌아오나요?" |

### 산출 — USER-PROFILE.md 생성

위치: **KM 단독 설치 = vault 루트 `_meta/USER-PROFILE.md`** (ThisCode/ThisCodex 통합 설치면 `~/.claude/USER-PROFILE.md`). frontmatter 스키마:

```yaml
---
schema_version: user-profile-v1
interviewed: {today}              # 재인터뷰 트리거 = 90일 경과 or 사용자 요청
completeness: full | partial
roles: []                        # A — 업무 2~3 + 주당 시간
workflows: []                    # B — 단계·도구·산출물(업무당 1블록)
pain_points: []                  # C — 사례 포함, 심각도
automation_wishes: []            # D
info_flow: {}                    # E — 수집처·검색 습관·재작성 사례
north_star: ""                   # F
---
```

- 본문 = 인터뷰 대화의 **요약 산문**(후속 답변의 뉘앙스 보존 — frontmatter는 기계용, 본문은 봇이 읽는 컨텍스트용).
- 생성 후 안내: "저장했어요. **이 답들은 노트의 구조·저장 방식(폴더·분할·연결)에 직접 반영됩니다** — 이제 `/knowledge-manager`로 정리할 때 매번 묻던 상세수준·구조·연결을 이 프로필에서 기본값으로 잡고 확인만 받습니다."

### 프로필 → KM STEP 1 기본값 유도 (소비처)
- `roles`/`info_flow` → 상세수준·중점영역·노트분할·연결수준 기본값. 예: info_flow가 "링크·PDF 다수·자주 못 찾음" → 연결수준=최대·분할=3-tier. roles가 "코드 작업 중심" → 중점=기술/코드.
- KM STEP 1은 프로필 있으면 4질문을 **"이 기본값으로 저장할까요?" 1질문**으로 축약(없으면 기존 4질문 fallback).
