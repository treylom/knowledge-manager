...
# 3순위: WebFetch (정적 콘텐츠 추출 가능)
WebFetch({ url: "[URL]", prompt: "전체 내용 추출" })

# 4순위: Hyperbrowser (클라우드, 크레딧 소진 주의)
...
## 2A. 웹 크롤링 (🚨 Playwright CLI 우선!)

### 브라우징 스택 우선순위 (CRITICAL!)

```
1순위: Playwright CLI (Bash 기반 - 가장 안정적, JS 렌더링 지원)
2순위: Playwright MCP (CLI 실패 시 - 동일 기능, MCP 프로토콜)
3순위: WebFetch (정적 콘텐츠 추출 가능 - 허용)
...
### 1순위: Playwright CLI 워크플로우 (🚨 MUST TRY FIRST!)

```bash
# Step 1: 브라우저 열기 + 페이지 이동
playwright-cli open "[URL]"

# Step 2: 동적 콘텐츠 대기 (소셜 미디어는 스크롤 추가)
playwright-cli press End           # 페이지 하단으로 스크롤 (선택)

# Step 3: 콘텐츠 추출 - 접근성 스냅샷
...
### 3순위: WebFetch (Playwright 모두 실패 시 - 허용)
...
> Playwright 실패 시 차선책으로 사용합니다.

### 웹 크롤링 완료 검증 (필수!)

```
□ playwright-cli snapshot 또는 browser_snapshot 호출 완료?
□ 스냅샷에서 실제 콘텐츠 확인 가능?
...
| 에러 | 대응 |
|------|------|
| CLI 미설치 | `playwright-cli install --skills` 실행 후 재시도 |
| 봇 감지/차단 | **스텔스 스크립트 사용** (아래 참조) |
| 콘텐츠 미로드 | 스크롤 + 재시도, WebFetch 폴백 |
| 네트워크 오류 | 지수 백오프로 재시도 |

### 봇 탐지 시 스텔스 스크립트 폴백

```bash
# 1. 브라우저 먼저 종료 (프로필 잠금 충돌 방지!)
playwright-cli close
