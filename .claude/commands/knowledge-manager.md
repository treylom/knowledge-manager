...
**⚠️ 콘텐츠 처리/읽기 전에 반드시 AskUserQuestion을 4번 순차적으로 호출하세요!**

### Question 1: 상세 수준 (Detail Level)
...
### Question 2: 중점 영역 (Focus Area)

```json
AskUserQuestion({
  "questions": [{
    "question": "어떤 영역에 중점을 둘까요? (복수 선택 가능, 선택 안 하면 전체 균형)",
    "header": "중점 영역",
    "options": [
      {"label": "개념/이론", "description": "핵심 아이디어와 원리"},
      {"label": "실용/활용", "description": "사용법, 예시, 튜토리얼"},
      {"label": "기술/코드", "description": "구현, 아키텍처, 코드"},
      {"label": "인사이트", "description": "시사점, 의견, 분석"}
    ],
    "multiSelect": true
  }]
})
```
> 아무것도 선택 안 하면 "전체 균형(E)" 자동 적용

### Question 3: 노트 분할 방식 (Note Structure)
...
### Question 4: 연결 수준 (Connection Level)
