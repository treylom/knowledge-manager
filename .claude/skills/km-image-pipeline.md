...
- 절대 경로 (Write/Bash 사용 시): `{vaultPath}/Resources/images/{topic-folder}/`
...
Write(file_path="{vaultPath}/Zettelkasten/[카테고리]/[노트명].md", content="[마크다운]")
```
...
```bash
# 1. 디렉토리 생성
mkdir -p {vaultPath}/Resources/images/{topic-folder}/

# 2. 웹 이미지 다운로드
curl -sLo "{vaultPath}/Resources/images/{topic-folder}/{NN}-{desc}.{ext}" "{url}"

# 3. PDF 이미지 복사
cp km-temp/{name}/images/{file} "{vaultPath}/Resources/images/{topic-folder}/{NN}-{desc}.{ext}"

# 4. 다운로드 실패 시 Playwright 스크린샷 폴백
mcp__playwright__browser_take_screenshot({
  ref: "{element-ref}",
  element: "{element-description}",
  filename: "{vaultPath}/Resources/images/{topic-folder}/{NN}-screenshot.{ext}"
})
```
