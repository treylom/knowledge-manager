...
> 💡 **다른 워커에게 결과를 넘길 때** — 문서 세 건 이상을 다른 워커에게 넘긴다면 목록만 던지지 말고 문서 사이 관계를 한 줄 붙여 주세요(`A=원인 · B=재현 · C=해결` 식). 받는 워커는 그 한 줄로 어디부터 열지를 정합니다. 그래프를 통째로 직렬화해 넘기면 오히려 무엇이 중요한지가 묻힙니다.
