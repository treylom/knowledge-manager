...
설치기는 심볼릭 링크를 따라가지 않고(링크가 있으면 멈춤), 검사가 끝나기 전에는 프로젝트에 아무것도 쓰지 않습니다 — 중간에 실패하면 프로젝트는 그대로입니다.
...
The installer never follows symbolic links (it stops if it finds one) and writes nothing into the project until every file has been staged and checked — if it fails midway, the project is unchanged.
